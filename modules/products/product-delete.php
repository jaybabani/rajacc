...
<div class="row row-cols-1 row-cols-sm-1 row-cols-md-1 row-cols-lg-1 row-cols-xl-1 row-cols-xxl-1 g-4 py-3 px-2">
    ...
    $titletag = "Delete Product";

    $tablename = "products";

    $msg = [
        "success_delete" => "Product deleted successfully",
        "error_delete" => "Error in deleting product",
    ];
    ...
    <form action="product-delete.php" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-9 col-sm-10 col-xs-12">
                    <div class="form-group">
                        <label class="form-label" for="field-4">Are You sure you want to delete product
                            <?php echo "ID: <strong>" . get_value($data, 'id') . "</strong>";
                            $product = name_title(get_value($data, 'product'));
                            echo " Product: <strong>" . $product . "</strong>"; ?>?</label>
                    </div>
                </div>
            </div>
        </div>
